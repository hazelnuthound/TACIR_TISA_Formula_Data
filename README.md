...
Many of the variables used in TACIR’s fiscal capacity model are expressed in terms of pupils.  As such, the most important variable to calculate is average daily membership.  Two scripts are supplied to calcuated ADM. The script "3_download_TN_Dept_Ed_data.R" will download Tennessee Department of Education (TDoE) Annual Statistical Reports in Excel format.  The script "4_calculate_ADM.R" processes the files and returns the a calculation of ADM using TACIR's three-year average methodology,

## Next Steps
The next script will use the TDoE data to calculate own-source revenue per pupil.
